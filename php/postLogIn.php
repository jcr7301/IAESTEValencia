...
<body>
	...
	<?php } else {
		...
		//$_POST[dni] = $mysqli->real_escape_string($_POST[dni]);
		//$_POST[password] = $mysqli->real_escape_string($_POST[password]);
		$result = mysqli_query($conn,"SELECT nombre, centro, grupo_trabajo, empresas_registradas, empresas_report, puntos FROM usuarios where dni='$_POST[dni]' and contrasena='$_POST[password]'");
		$datos_empresa = mysqli_query($conn, "SELECT cif, nombre, especialidad, direccion, contacto, email, telefono FROM empresas where asignada_a='$_POST[dni]' and enviada='No'");
