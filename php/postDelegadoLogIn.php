...
<body>
	...
	<?php } else {
		...
		$dni_delegado = mysqli_real_escape_string($conn, $_POST['dni_delegado']);
		$password_delegado = mysqli_real_escape_string($conn, $_POST['password_delegado']);
		$result = mysqli_query($conn,"SELECT nombre, centro, grupo_trabajo, empresas_registradas, empresas_report, puntos, delegado FROM usuarios where dni=$dni_delegado and contrasena=$password_delegado and delegado='Si'");
		$datos_empresas_asignadas = mysqli_query($conn, "SELECT cif, nombre, especialidad, direccion, contacto, email, telefono FROM empresas where asignada_a=$dni_delegado and enviada='No'");
